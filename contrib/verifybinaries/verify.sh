...
###   erexcoincore.org and erexcoin.org and compares them.
...
WORKINGDIR="/tmp/erexcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://erexcoincore.org"
HOST2="https://erexcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="erexcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'erexcoin-',
...
if [ $? -ne 0 ]; then
   echo "erexcoin.org failed to provide signature file, but erexcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "erexcoin.org and erexcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the ErexCoin Core binary release signing key installed?"
   fi
