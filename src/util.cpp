...
const char * const EREXCOIN_CONF_FILENAME = "erexcoin.conf";
const char * const EREXCOIN_PID_FILENAME = "erexcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "erexcoind -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ErexCoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EREXCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", EREXCOIN_PID_FILENAME)));
}
